...
const Navbar = ({statePage, setStatePage}) => {
  ...
  return (
    <nav className="w-full flex py-6 justify-between items-center navbar">
      ...
      >
        <h1
          className={`font-poppins font-semibold cursor-pointer text-[24px] mr-10  bg-blue-green-gradient-text`}
        >{`{JG}`}</h1>
      </div>
        ...
        {navLinks.map((nav, index) => (
          <li
            ...
            className={`font-poppins  cursor-pointer text-[16px] ${
              ...
              statePage == nav.id
                ? "font-semibold bg-blue-gradient-text"
        ...
        >
          <ul className="list-none flex flex-col justify-end items-start flex-1">
            {navLinks.map((nav, index) => (
              ...
              >
                <a onClick={(event) => handleLinkClick(event, nav)}>
                  {nav.title}
                </a>
              </li>
            ))}
